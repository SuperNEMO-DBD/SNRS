...
namespace snrs {
  ...
  void ltd_zband::reset()
  {
    ...
    xmin = datatools::invalid_real();
    xmax = datatools::invalid_real();
    ymin = datatools::invalid_real();
    ymax = datatools::invalid_real();
    point_ids.clear();
  }

  // friend
  std::ostream & operator<<(std::ostream & out_, const ltd_zband & zband_)
  {
    out_ << "{id=" << zband_.id << ",z=" << zband_.z << ",dz=" << zband_.dz << ",xmin=" << zband_.xmin << ",xmax=" << zband_.xmax << ",ymin=" << zband_.ymin << ",ymax=" << zband_.ymax << ",npoints=" << zband_.point_ids.size() << "}";
    return out_;
  }
  ...
  void ltd::extract_zband_min_max(double zmin_, double zmax_, ltd_zband & zband_) const
  {
    double z = 0.5 * (zmin_ + zmax_);
    double dz = 0.5 * std::abs(zmax_ - zmin_);
    extract_zband(z, dz, zband_);
  }
  ...
  void ltd::extract_zband(double z_, double dz_, ltd_zband & zband_) const
  {
    ...
    for (uint32_t i = 0; i < _points_.size(); i++) {
      ...
      if (std::abs(z_ - pi.z()) < dz_) {
        ...
        if (not datatools::is_valid(zband_.xmin) or pi.x() < zband_.xmin) {
          zband_.xmin = pi.x();
        }
        if (not datatools::is_valid(zband_.xmax) or pi.x() > zband_.xmax) {
          zband_.xmax = pi.x();
        }
        if (not datatools::is_valid(zband_.ymin) or pi.y() < zband_.ymin) {
          zband_.ymin = pi.y();
        }
        if (not datatools::is_valid(zband_.ymax) or pi.y() > zband_.ymax) {
          zband_.ymax = pi.y();
        }
      }
    }
  ...
  void ltd::y_cut_zband(double ymin_, double ymax_, ltd_zband & zband_) const
  {
  ...
  void ltd::generate_y_cut_zband(double ymin_, double ymax_,
                                 const ltd_zband & source_, ltd_zband & target_) const
  {
    DT_THROW_IF(&source_ == &target_, std::logic_error, "Self reference is not allowed!");
    target_.reset();
    target_.id = source_.id;
    target_.z  = source_.z;
    target_.dz = source_.dz;
    for (std::set<uint32_t>::const_iterator it = source_.point_ids.begin();
         it != source_.point_ids.end();
         it++) {
      int i = *it;
      const geomtools::vector_3d & pi = _points_[i];
      if (pi.y() >= ymin_ and pi.y() <= ymax_) {
        target_.point_ids.insert(i);
        if (not datatools::is_valid(target_.xmin) or pi.x() < target_.xmin) {
          target_.xmin = pi.x();
        }
        if (not datatools::is_valid(target_.xmax) or pi.x() > target_.xmax) {
          target_.xmax = pi.x();
        }
        if (not datatools::is_valid(target_.ymin) or pi.y() < target_.ymin) {
          target_.ymin = pi.y();
        }
        if (not datatools::is_valid(target_.ymax) or pi.y() > target_.ymax) {
          target_.ymax = pi.y();
        }
      }
    }
